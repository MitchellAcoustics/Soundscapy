...
from scipy.stats.tests.test_continuous_basic import check_distribution_rvs
...
from scipy.stats.tests.common_tests import check_random_state_property
from scipy.stats.tests.data._mvt import _qsimvtv
...
class TestMultivariateSkewNormal:
    ...
    def test_degenerate_distributions(self):
        for n in range(1, 5):
            ...
            for k in range(1, n):
                ...
                # Check some identities.
                distn_kk = multivariate_skewnormal(
                    np.zeros(k), cov_kk, allow_singular=True
                )
                distn_nn = multivariate_skewnormal(
                    np.zeros(n), cov_nn, allow_singular=True
                )
                distn_rr = multivariate_skewnormal(
    ...
    def test_degenerate_array(self):
        ...
        for n in range(2, 6):
            for r in range(1, n):
                ...
                pdf = multivariate_skewnormal.pdf(
                    X, mean=mn, cov=vr, allow_singular=True
                )
                assert_equal(pdf.size, k)
                assert np.all(pdf > 0.0)

                logpdf = multivariate_skewnormal.logpdf(
                    X, mean=mn, cov=vr, allow_singular=True
                )
                assert_equal(logpdf.size, k)
                assert np.all(logpdf > -np.inf)
    ...
    def test_frozen(self):
        ...
        assert_allclose(
            skewnorm_frozen.pdf(x), multivariate_skewnormal.pdf(x, mean, cov, skew)
        )
        assert_allclose(
            skewnorm_frozen.logpdf(x),
            multivariate_skewnormal.logpdf(x, mean, cov, skew),
        )
        assert_allclose(
            skewnorm_frozen.cdf(x), multivariate_skewnormal.cdf(x, mean, cov, skew)
        )
        assert_allclose(
            skewnorm_frozen.logcdf(x),
    ...
    def test_rvs_shape(self):
        ...
        N = 300
        d = 4
        sample = multivariate_skewnormal.rvs(
            mean=np.zeros(d), cov=1, skew=np.zeros(d), size=N
        )
        assert_equal(sample.shape, (N, d))

        sample = multivariate_skewnormal.rvs(
            mean=None, cov=np.array([[2, 0.1], [0.1, 1]]), skew=None, size=N
        )
        assert_equal(sample.shape, (N, 2))

...
import numpy as np
import pytest
from soundscapy.audio.metric_registry import stat_calcs


class TestMetricsUnit(unittest.TestCase):
    def teststat_calcs_avg(self):
        ts = np.array([1, 2, 3, 4, 5])
        res = {}
        updated_res = stat_calcs("metric", ts, res, ["avg"])
        self.assertEqual(
            updated_res["metric_avg"], 3.0, "Average calculation is incorrect"
        )

    def teststat_calcs_percentile(self):
        ts = np.array([1, 2, 3, 4, 5])
        res = {}
        updated_res = stat_calcs("metric", ts, res, [50])
        self.assertEqual(
            updated_res["metric_50"], 3.0, "50th percentile calculation is incorrect"
        )

    def teststat_calcs_max(self):
        ts = np.array([1, 2, 3, 4, 5])
        res = {}
        updated_res = stat_calcs("metric", ts, res, ["max"])
        self.assertEqual(
            updated_res["metric_max"], 5, "Max value calculation is incorrect"
        )

    def teststat_calcs_multiple_stats(self):
        ts = np.array([1, 2, 3, 4, 5])
        res = {}
    ...
    def teststat_calcs_empty_ts(self):
        ts = np.array([])
        res = {}
        updated_res = stat_calcs("metric", ts, res, ["avg"])
        self.assertTrue(

...
from scipy.stats._continuous_distns import _norm_cdf, _norm_logcdf
from scipy.stats._discrete_distns import binom
from scipy.stats import _mvn, _covariance, _rcont
from scipy.stats._qmvnt import _qmvt
from scipy.stats._morestats import directional_stats
...
class multivariate_skewnormal_gen(multi_rv_generic):
    ...
    def __call__(
        self,
        loc=None,
        cov: np.ndarray[Any, np.dtype[Any]] | int = 1,
        skew=None,
        allow_singular=False,
        seed=None,
    ):
        ...
        """
        return multivariate_skewnormal_frozen(
            loc, cov, skew, allow_singular=allow_singular, seed=seed
        )
    ...
    def _process_parameters_Covariance(self, loc, cov, skew):
        dim = cov.shape[-1]
        loc = np.array([0.0]) if loc is None else loc
        skew = np.array([0.0]) if skew is None else skew
    ...
    def _process_parameters_psd(self, dim, loc, cov, skew):
        ...
        if skew.ndim != 1 or skew.shape[0] != dim:
            ...
            raise ValueError("Array 'skew' must be a vector of length %d." % dim)
        if cov.ndim == 0:
            cov = cov * np.eye(dim)
        elif cov.ndim == 1:
            cov = np.diag(cov)
    ...
    def logpdf(self, x, loc=None, cov=1, skew=None, allow_singular=False):
        ...
        %(_mvsn_doc_callparams_note)s
        """
        dim, loc, cov_object, skew = self._process_parameters(
            loc, cov, skew, allow_singular
        )
        x = self._process_quantiles(x, dim)
    ...
    def logcdf(
        ...
        xi = np.insert((x - loc) / omega, 0, 0.0, axis=-1)
        zeros = np.zeros(shape=(1 + dim))

        out = np.log(self._cdf(xi, zeros, bigO, maxpts, abseps, releps, lower_limit))
        return out
...
class multivariate_skewnormal_frozen(multi_rv_frozen):
    def __init__(
        ...
        self.dim, self.loc, self.cov_object, self.skew = self._dist._process_parameters(
            loc, cov, skew, allow_singular
        )
        self.allow_singular = allow_singular or self.cov_object._allow_singular
        if not maxpts:
    ...
    def rvs(self, size=1, random_state=None):
        return self._dist.rvs(self.loc, self.cov, self.skew, size, random_state)
...
for name in ["logpdf", "pdf", "logcdf", "cdf", "rvs"]:
    ...
    method_frozen.__doc__ = doccer.docformat(method.__doc__, mvsn_docdict_noparams)
...
def _dirichlet_check_parameters(alpha):
    ...
    if np.min(alpha) <= 0:
        ...
        raise ValueError(
            "Parameter vector 'a' must be one dimensional, but a.shape = {}.".format(
                alpha.shape
            )
        )
    return alpha
...
class wishart_gen(multi_rv_generic):
    ...
    def _process_quantiles(self, x, dim):
        ...
        if not x.shape[0:2] == (dim, dim):
            raise ValueError(
                "Quantiles have incompatible dimensions: should be {}, got {}.".format(
                    (dim, dim), x.shape[0:2]
                )
            )

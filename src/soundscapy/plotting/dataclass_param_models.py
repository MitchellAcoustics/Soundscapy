...
class ParamModel:
    ...
    def update(
        ...
        **kwargs : Any
        ...
        if extra == "forbid":
            # Forbid extra fields
            unknown_keys = set(kwargs) - set(self.defined_field_names)
            if unknown_keys:
                msg = f"Unknown parameters: {unknown_keys}"
                raise ValueError(msg)
        elif extra == "ignore":
            # Ignore extra fields
            kwargs = {k: v for k, v in kwargs.items() if k in self.defined_field_names}
        ...
        for key, value in update_kwargs.items():
            if key in self.defined_field_names:
                setattr(self, key, value)
            elif extra == "allow":
                self._extra_fields[key] = value
